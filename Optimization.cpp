...
namespace Optimization {
	...
	void Bfgs::updateDx()
	{
		if( itr == 0 ) {
			...
			alpha = linearSearch(g0);
			dx = - alpha * g0;                            Logger::out(2) << "dx=" << dx << std::endl;
			...
			H1 = prod(trans(B), matrix(prod(H0, B))) + A; Logger::out(2) << "H1=" << H1 << std::endl;
			H0 = H1;
			vector d = prod(H1, g0);
	...
	public:
		...
		virtual vector grad(vector& x){
			...
			g(0) = x[0]-x[1]*x[1];
			double a = (x[1]-2);
			g(1) = -0.5*x[1]*(x[0]-x[1]*x[1])+a*a*a;
			return std::move(g);
		}
		virtual void update(const vector& dx){
			x0 += dx[0];
			x1 += dx[1];
		}
